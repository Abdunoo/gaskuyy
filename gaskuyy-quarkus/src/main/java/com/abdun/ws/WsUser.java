...
public class WsUser {
	...
	@Path("delete")
	@GET
	public void deleteUserByKey(@QueryParam("password") String password, @QueryParam("key") String key) {
		if (password.matches("panemujaya")) {
			RcdUser rcdUser = srvUser.findByUniqueKey(key);
			if (rcdUser != null) {
				...
				throw ErrorCode.ER0000.exception("user with key " + key + " not found");
			}
		} else {
			throw ErrorCode.ER0000.exception("password to delete user false");
		}
	}
